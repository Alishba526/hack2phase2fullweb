...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        print("Application will continue but database features may not work properly")
...
def create_app():
    ...
    app.add_middleware(
        ...
        allow_origins=[
            ...
            "http://127.0.0.1:3000",
            "http://localhost:5173",
            "https://alishbarehman-fullbackend.hf.space",
        ],  # Allow specific origins + Vercel
        allow_credentials=True,
        allow_methods=["*"],
        allow_headers=["*"],
        # Allow credentials to be sent with cross-origin requests
        allow_origin_regex="https://.*\.vercel\.app",  # Allow all Vercel preview/production URLs
